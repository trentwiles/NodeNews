...
async function init() {
  await query(
    "CREATE TABLE IF NOT EXISTS eml (email VARCHAR(255) PRIMARY KEY, ts INTEGER)"
  );
  await query(
    "CREATE TABLE IF NOT EXISTS users (username VARCHAR(255) PRIMARY KEY, password VARCHAR(255) NOT NULL, permissions VARCHAR(1), joinTime INTEGER, lastAccessedTime INTEGER)"
  );
  await query(
    "CREATE TABLE IF NOT EXISTS tokens (tkn VARCHAR(255) PRIMARY KEY, username VARCHAR(255), ts INTEGER, FOREIGN KEY (username) REFERENCES users(username))"
  );
